...
**Функция роста** $m_H(N) = max_{x_1..x_N} |H(X_1,..X_N)| \leq 2^N$ Максимальное количество дихотомий на N точках при классе гипотез H

**Точка поломки** $min(k : m_H(k) < 2^k)$. Минимальное число точек, на котором $2^k$ не достигается

Неравенство Вапника-Червоненко (VS Inequality) $P(|E_{in}(h) - E_{out}(h)| > \epsilon) \leq m_H(2N)\cdot 4e^{-\epsilon N/8}$
